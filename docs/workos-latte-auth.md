...
- **Latte** is the React browser app template. It checks the current session, redirects the browser to WorkOS AuthKit, receives the callback route in the browser, and forwards callback parameters to Pane through its `/pane` proxy.
...
- Latte proxies API calls from `/pane/*` to Pane's Nginx service on the Docker network.
...
Latte's Docker env normally contains:
